...
class CustomMainMenuScreen extends Module
{
  ...
  function onPlayStateExit(state:PauseSubState):Void
  {
  ...
  override function onUpdate(event:UpdateScriptEvent)
  {
    ...
    if (freeplayInitDone && observer_value)
    {
      // `prevOpange` got shown AND hidden
      //* We are closing freeplay now
      FlxTimer.globalManager.clear();

      freeplayInitDone = false;
      freePlay.backingCard.cardGlow.visible = false;
      trace("Closing");
    ...
    else if (!freeplayInitDone && observer_value)
    {
      // We are done initing
      trace("Done initting");
      freeplayInitDone = true;
      freePlay.backingCard.cardGlow.visible = false;
    }
  }
}
