...
class CustomUI extends Module
{
  ...
  override function onSongLoaded(event:SongLoadScriptEvent)
  {
    ...
    rankSprite = FunkinSprite.create(1010, uiY = game.scoreText.y - 57, "p-rank");
    rankSprite.cameras = [game.camHUD];
    rankSprite.setGraphicSize(Std.int(rankSprite.width * 0.5));
    rankSprite.updateHitbox();
    rankSprite.visible = false;
    game.add(rankSprite);

    rankPrefectSprite = FunkinSprite.createSparrow(966, rankSprite.y - 90, "p-rank-gold");
    rankPrefectSprite.animation.addByPrefix('gold', 'p-rank-gold', 24, true);
    rankPrefectSprite.animation.play('gold', true);
    rankPrefectSprite.cameras = [game.camHUD];
    rankPrefectSprite.setGraphicSize(Std.int(rankPrefectSprite.width * 0.8));
    rankPrefectSprite.updateHitbox();
    rankPrefectSprite.visible = true;
    game.add(rankPrefectSprite);
  ...
  function updateScoreText():Void
  {
    ...
    missedText.scriptCall("displayText", [Std.string(Highscore.tallies.missed), false]);
  }
  ...
  function recalculateRating()
  {
    ...
    var isSFC = scoreData.good + scoreData.bad + scoreData.shit + scoreData.missed == 0;
  ...
  override function onNoteHit(event:HitNoteScriptEvent)
  {
    ...
    if (game.isPracticeMode && game.isBotPlayMode) return;
    new FlxTimer().start(0.01, (_) -> recalculateRating());
  }

  override function onNoteMiss(event:NoteScriptEvent)
  {
    new FlxTimer().start(0.01, (_) -> recalculateRating());
    // Ensure, that the current note is counted too...
